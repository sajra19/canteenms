...
class Validator {
    ...
    /**
     * Validate pagination.
     * Validate proper pagination for listing requests.
     *
     * @static
     * @return array Page number and page limit.
     */
    public static function validate_pagination() {
        $query_params = \Flight::request()->query->getData();
        /* Default page */
        $page = 1;
        if (isset($query_params['page']) && $query_params['page'] > 0) {
            $page = (int) $query_params['page'];
        }
        /* Default limit */
        $limit = 25;
        if (isset($query_params['limit']) && $query_params['limit'] > 0 && $query_params['limit'] < 1000) {
